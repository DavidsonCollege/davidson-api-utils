...
class QueryBuilder {
    ...
    selectRaw (raw) {
        this.action = 'select'
        // if Raw is passed in
        if (typeof raw === 'object' && raw.constructor.name === 'Raw') {
            this.selections = raw.sql
        }
    }
    ...
    whereSql (wheres, root = true) {
        ...
        for (let i = 0;  i < wheres.length; i++) {
            ...
            if (wheres[i].group) {
                sql += `${wheres[i].not}(${this.whereSql(wheres[i].wheres, false)})`
            } else {
                sql += wheres[i].sql
            }
        }
    ...
    getCrossApplys (wheres) {
        ...
        // make unique
        // join with spaces
        return wheres.reduce((allWheres, currentWhere) => {
            // if curr is an array, this is a whereGroup...
            const crossApplies = Array.isArray(currentWhere)
                // ... it needs to be recursively assessed
                ? this.getCrossApplys(currentWhere)
                // else get the curr.json.xApplies out of it or an empty array
                : currentWhere.column.json?.xApplies || []
            // concat the return to the accumulating array of cross applies
            return allWheres.concat(crossApplies)
