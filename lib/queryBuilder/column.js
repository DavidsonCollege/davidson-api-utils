...
class Column {
    ...
    processJsonPath(string) {
        ...
        if (string.indexOf('*') === -1) {
            ...
            this.json.assignment = `JSON_${is_object ? 'QUERY' : 'VALUE'}(${this.json.root}, '$${subPath}')`
            this.alias = this.normalizeJsonPath(string)
        }
        // else, things get a little more complex
            ...
            for (let i = 0; i < splatSplits.length; i++) {
                ...
                const crossApply = `CROSS APPLY OPENJSON(${is_first ? this.json.root : freezeComputedXApply}, '$.${subPath}') WITH (${computedXApply} NVARCHAR(max) '$'${is_object ? ' AS JSON' : ''})`
                // add the cross apply sql to the array
                this.json.xApplies.push(crossApply)
                if (is_last) {
                    this.json.assignment = computedXApply
                    this.alias = computedXApply.replace(/^x_/, '')
                }
            }
        }
